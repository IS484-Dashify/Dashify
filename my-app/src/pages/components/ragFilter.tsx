...
const RagFilter: React.FC<{
  filterSettings: Array<string>;
  handleFilterClick: (filter: string) => void;
}> = ({ filterSettings, handleFilterClick }) => {
  const options = [{ option: "Critical" }, { option: "Warning" }, { option: "Normal" }];
  const [filterText, setFilterText] = useState<string>("Critical, Warning, Normal");
  useEffect(() => {
    let result = [];
        ...
        >
          <div className="flex flex-col gap-2.5">
            <div className="flex justify-between align-middle">
              <p className="text-pri-500 text-[15px] font-medium align-bottom">
                Filters
              </p>
              <Popover.Close
            ...
            {options.map((option, index) => (
              <fieldset className="flex gap-5 items-center" key={index}>
                <Checkbox.Root
                  className="h-[25px] w-[25px] appearance-none flex items-center justify-center rounded-[4px] bg-white border-1 border-text/20 shadow-lg hover:border-pri-500 transition-colors duration-200 ease-soft-spring"
                  id={`${option.option}Filter`}
                  value={option.option}
                ...
                <label
                  className="-ml-2 text-[15px] leading-none text-text hover:text-pri-500 transition-colors duration-200 ease-soft-spring cursor-pointer"
                  htmlFor={`${option.option}Filter`}
                >
